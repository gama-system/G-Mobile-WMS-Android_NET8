...
namespace G_Mobile_Android_WMS
{
    ...
    public class InfoActivity : BaseWMSActivity
    {
        ...
        private void GetAndSetControls()
        {
            ...
            Helpers.SetTextOnTextView(this, InfoWersja, GetString(Resource.String.info_activity_wersja) + " " + Globalne.AppVer);

            if (Globalne.Licencja != null)
            {
                InfoZarejestrowanoDla = FindViewById<TextView>(Resource.Id.InfoZarejestrowanoDlaX);
                Helpers.SetTextOnTextView(this, InfoZarejestrowanoDla, Globalne.Licencja.FirmaLicencjonowana);

                InfoCzasLicencji = FindViewById<TextView>(Resource.Id.InfoCzasLicencjiX);

                if (Globalne.Licencja.LicencjaCzasowa)
                    Helpers.SetTextOnTextView(this, InfoCzasLicencji, Globalne.Licencja.PoczLicencji.ToString(Globalne.CurrentSettings.DateFormat) + " - " + Globalne.Licencja.TerminLicencji.ToString(Globalne.CurrentSettings.DateFormat));
                else
                    Helpers.SetTextOnTextView(this, InfoCzasLicencji, GetString(Resource.String.info_activity_licencjapelna));
            }
        ...
        private void InfoBtnSettings_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                i.PutExtra(UsersActivity.Vars.Mode, (int)UsersActivity.UsersActivityModes.IsAllowedToEditSettings);

                RunOnUiThread(() => StartActivityForResult(i, (int)ResultCodes.CanEditSettingsResult));
            }
        }
        ...
        {
            RunIsBusyAction(() => Helpers.SwitchAndFinishCurrentActivity(this, typeof(MainActivity)));
        }
    }
}
