...
namespace G_Mobile_Android_WMS.BusinessLogicHelpers
{
    class DocumentItems
    {
        ...
        {
            return (
                (DocType == DocTypes.ZLGathering || DocType == DocTypes.MMGathering)
                || (DocType == DocTypes.ZL && Operation == Operation.Out)
                || (DocType == DocTypes.MM && Operation == Operation.Out)
            );
        }
        ...
        {
            return (
                (DocType == DocTypes.ZLDistribution || DocType == DocTypes.MMDistribution)
                || (DocType == DocTypes.ZL && Operation == Operation.In)
                || (DocType == DocTypes.MM && Operation == Operation.In)
            );
        }
        ...
        public static async Task<bool> DeleteDocumentItems(
            Context ctx,
            List<int> DocItems,
        ...
        )
        {
            ...
            if (DocItems.Count == 1)
            {
                bool Res = await Helpers.QuestionAlertAsync(
                    ctx,
                    Resource.String.editingdocuments_deleteitem_confirm,
            ...
            else
            {
                if (
                    !await Helpers.QuestionAlertAsync(
                        ctx,
                        Resource.String.editingdocuments_deleteitem_many,
                        Resource.Raw.sound_message
                    )
                    || !await Helpers.QuestionAlertAsync(
                        ctx,
                        Resource.String.editingdocuments_deleteitem_many_confirm,
                ...
                else
                {
                    ...
                    if (ErrorsHappened)
                        await Helpers.AlertAsyncWithConfirm(
                            ctx,
                            Resource.String.editingdocuments_cannot_deleteitem_many
                        );
                ...
                try
                {
                    ...
                    switch (TargetProperty)
                    {
                        case nameof(Item.ExIDArticle):
                        {
                            ...
                            System.Reflection.PropertyInfo pi2 = Kod.GetType()
                        ...
                        {
                            System.Reflection.PropertyInfo pi1 = Item.GetType()
        ...
        public static void SetLogisticFunction(
            Activity Ctx,
            DokumentVO Doc,
            ref DocumentItemVO Item
        )
        {
            try
            {
...
#warning HiveInvoke
        ...
        )
        {
            ...
            Item.ExIDUnit = Convert.ToInt32(
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idJednostki
                ]
            );
            Item.ExUnit = (string)
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strNazwaJednostki
                ];
            Item.ExIDPartia = Convert.ToInt32(
                Zap.ListaWierszy[0][(int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idPartii]
            );
            Item.ExPartia = (string)
                Zap.ListaWierszy[0][(int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strPartia];
            Item.ExIDArticle = Convert.ToInt32(
                Zap.ListaWierszy[0][(int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idTowaru]
            );
            Item.ExArticle = (string)
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strNazwaTowaru
                ];

            if (CurrentOperation == Operation.In || CurrentOperation == Operation.OutIn)
                Item.ExIDFunkcjaLogistycznaP = Convert.ToInt32(
                    Zap.ListaWierszy[0][
                        (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idFunkcjiLogistycznej
                    ]
                );
            if (CurrentOperation == Operation.Out || CurrentOperation == Operation.OutIn)
                Item.ExIDFunkcjaLogistycznaW = Convert.ToInt32(
                    Zap.ListaWierszy[0][
                        (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idFunkcjiLogistycznej
                    ]
                );
            ...
            Item.ExLot = (string)
                Zap.ListaWierszy[0][(int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strLoty];
            Item.ExSerialNum = (string)
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strNumerySeryjne
                ];
            Item.ExIDOwner = Convert.ToInt32(
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idKontrahenta
                ]
            );
            Item.ExOwner = (string)
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strNazwaKtr
                ];
            Item.ExProductionDate = (DateTime)
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.dtDataProdukcji
                ];
            Item.ExBestBefore = (DateTime)
                Zap.ListaWierszy[0][
                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.dtDataPrzydatnosci
                ];

            if (CurrentOperation == Operation.In || CurrentOperation == Operation.OutIn)
                Item.ExIDPaletaP = Convert.ToInt32(
                    Zap.ListaWierszy[0][(int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idPalP]
                );
            if (CurrentOperation == Operation.Out || CurrentOperation == Operation.OutIn)
                Item.ExIDPaletaW = Convert.ToInt32(
                    Zap.ListaWierszy[0][(int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idPalP]
                );
        }
        ...
        public static void InsertSSCCData(ref KodKreskowyZSzablonuO Kod)
        {
            if (Kod.Paleta != "")
            {
                ...
                if (IDPozPal >= 0)
                {
                    ...
                    Kod.Partia = (string)
                        Zap.ListaWierszy[0][
                            (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.strPartia
                        ];
                    ...
                    Kod.TowaryJednostkiWBazie.Add(
                        new TowarJednostkaO()
                        {
                            IDTowaru = Convert.ToInt32(
                                Zap.ListaWierszy[0][
                                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idTowaru
                                ]
                            ),
                            IDJednostki = Convert.ToInt32(
                                Zap.ListaWierszy[0][
                                    (int)SQL.DocumentItems.Paleta_GetFirstInData_Results.idJednostki
                                ]
                            )
        ...
        public static bool GetSSCCData(
            string Paleta,
            ref DocumentItemVO Item,
            Operation CurrentOperation
        )
        ...
        private static decimal GetInWarehouse_OnItem_OutOperation(
            DokumentVO Doc,
            DocumentItemVO Item,
        ...
        public static void ProposeLocations(
            ref DokumentVO Doc,
            ref DocumentItemVO Item,
            Operation CurrentOperation,
        ...
        )
        {
            if (
                ...
                && (DefLocType == DefaultLocType.In || DefLocType == DefaultLocType.None)
            )
            {
            ...
            if (
                ...
                && (DefLocType == DefaultLocType.Out || DefLocType == DefaultLocType.None)
            )
            {
        ...
        public static async void EditItem(
            BaseWMSActivity ctx,
        ...
        )
        {
            ...
            if (
                DocType == DocTypes.MM
                || DocType == DocTypes.ZL
                || IsGatheringMode(DocType, CurrentOperation)
                || IsDistributionMode(DocType, CurrentOperation)
            )
                EditItemZLMM(
                    ctx,
                    Documents,
        ...
        public static void EditItemPWPZRWWZIN(
            BaseWMSActivity ctx,
            List<DokumentVO> Documents,
            DocumentItemRow SelectedItem,
            DocTypes DocType,
                        ...
                        if (EItem.Base.numIloscZrealizowana <= 0)
                        {
                            if (CurrentOperation == Operation.In)
                            {
                                ...
                                if (EItem.DefaultAmount == -1)
                                    EItem.DefaultAmount = GetDefaultValueByDocType(
                                        Doc,
                                        DocType,
                                        EItem,
                                        true
        ...
        public static void EditItemZLMM(
            BaseWMSActivity ctx,
            List<DokumentVO> Documents,
            DocumentItemRow SelectedItem,
            DocTypes DocType,
            Operation CurrentOperation,
            int SelectedDefaultLoc,
            Enums.DefaultLocType DefLocType,
            string SelectedDefaultLocName,
            int ResultCode,
            bool Add,
            bool FromScanner,
        ...
        )
        {
            try
            {
                ...
                i.PutExtra(
                    ...
                    (int)Enums.ItemActivityMode.Create
                );
                i.PutExtra(DocumentItemActivity_Common.Vars.Operation, (int)CurrentOperation);
                i.PutExtra(DocumentItemActivity_Common.Vars.FromScanner, FromScanner);
        ...
        private static decimal GetDefaultValueByDocType(
            DokumentVO Doc,
            DocTypes DocType,
            DocumentItemVO itemVO,
            bool skipSearchInLocation = false
        )
        {
            ...
            switch (DocType)
            {
                case DocTypes.PW:
                    if (Doc.bZlecenie)
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnOrderDocPW == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                            Doc,
                                            itemVO,
                                            skipSearchInLocation
                                        )
                                )
                                : Globalne.CurrentSettings.DefaultValueOnOrderDocPW;
                    else
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnDocPW == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnDocPW;
                    break;
                case DocTypes.RW:
                    if (Doc.bZlecenie)
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnOrderDocRW == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnOrderDocRW;
                    else
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnDocRW == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnDocRW;
                    break;
                case DocTypes.PZ:
                    if (Doc.bZlecenie)
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnOrderDocPZ == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnOrderDocPZ;
                    else
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnDocPZ == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnDocPZ;
                    break;
                case DocTypes.WZ:
                    if (Doc.bZlecenie)
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnOrderDocWZ == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnOrderDocWZ;
                    else
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnDocWZ == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnDocWZ;
                    break;
                case DocTypes.MM:
                ...
                case DocTypes.ZLDistribution:
                    if (Doc.bZlecenie)
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnOrderDocZL == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnOrderDocZL;
                    else
                        numIloscZlecona =
                            Globalne.CurrentSettings.DefaultValueOnDocZL == -1
                                ? (
                                    itemVO.Base.numIloscZlecona == 0
                                        ? 1
                                        : GetInWarehouse_OnItem_OutOperation(
                                ...
                                : Globalne.CurrentSettings.DefaultValueOnDocZL;
                    break;
                case DocTypes.IN:
                    break;
                case DocTypes.Error:
        ...
        public static void EditSplitItem(
            Activity ctx,
            PozycjaVO Original,
            bool Change,
            DocTypes DocType,
        ...
        )
        {
            ...
            if (Change)
            {
                ...
                if (BusinessLogicHelpers.DocumentItems.IsGatheringMode(DocType, Operation))
                    ...
                    Operation == Operation.OutIn
                    || BusinessLogicHelpers.DocumentItems.IsDistributionMode(DocType, Operation)
                )
                {
                    NewItem.numIloscZlecona = Original.numIloscZlecona - Amount;
                    NewItem.numIloscZebrana =
                        Original.numIloscZebrana - Amount < 0
                            ? 0
                            : Original.numIloscZebrana - Amount;
