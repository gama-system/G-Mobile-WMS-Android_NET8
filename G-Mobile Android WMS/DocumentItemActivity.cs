...
namespace G_Mobile_Android_WMS
{
    ...
    public class DocumentItemActivity : ActivityWithScanner
    {
        ...
        bool IsBlockedPosition 
        {
            get
            {
                if (Item != null && Item.Base != null)
                    return Item.Base.dataModyfikacji != Item.Base.dataUtworzenia && Item.Base.numIloscZrealizowana == Item.Base.numIloscZlecona;
                return false;
            }
            set { return; }
        }
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            DocType = (DocTypes)Intent.GetIntExtra(DocumentItemActivity_Common.Vars.DocType, (int)DocTypes.Error);
            Dokument = (DokumentVO)Helpers.DeserializePassedJSON(Intent, DocumentItemActivity_Common.Vars.DocJSON, typeof(DokumentVO));
            Item = (ExtendedModel.DocumentItemVO)Helpers.DeserializePassedJSON(Intent, DocumentItemActivity_Common.Vars.ItemJSON, typeof(ExtendedModel.DocumentItemVO));
            Mode = (ItemActivityMode)Intent.GetIntExtra(DocumentItemActivity_Common.Vars.Mode, 0);
            Operation = (Operation)Intent.GetIntExtra(DocumentItemActivity_Common.Vars.Operation, 0);
            BufferSet = Intent.GetBooleanExtra(DocumentItemActivity_Common.Vars.BufferSet, false);
            FromScanner = Intent.GetBooleanExtra(DocumentItemActivity_Common.Vars.FromScanner, false);
        ...
        private void SetupFields()
        {
            ...
            Flog.Text = Operation == Operation.In ? Item.ExFunkcjaLogistycznaP : Item.ExFunkcjaLogistycznaW;
            Flog.Tag = Operation == Operation.In ? Item.ExIDFunkcjaLogistycznaP : Item.ExIDFunkcjaLogistycznaW;
            EditFlog.Click += EditFlog_Click;

            Location.Text = Operation == Operation.In ? Item.ExLokalizacjaP : Item.ExLokalizacjaW;
            Location.Tag = Operation == Operation.In ? Item.ExIDLokalizacjaP : Item.ExIDLokalizacjaW;
            ...
            var przychod = Globalne.przychrozchBL.PobierzPrzychód(Operation == Operation.In ? Item.ExIDLokalizacjaP : Item.ExIDLokalizacjaW, Item.Base.idTowaru);
            if (przychod.ID > -1)
            {
            ...
            OnDoc.Text = Math.Round(Item.Base.numIloscZrealizowana, Globalne.CurrentSettings.DecimalSpaces).ToString("F3");
            ...
            if (Dokument.bZlecenie)
            {
                ...
                if (Operation == Operation.In ? (Item.Base.idPaletaP >= 0) : (Item.Base.idPaletaW >= 0))
                {
                ...
                if (!Globalne.CurrentSettings.AllowSourceLocationChange && ((Operation == Operation.In ? (Item.Base.idLokalizacjaP >= 0) : (Item.Base.idLokalizacjaW >= 0)) || (!Globalne.CurrentSettings.Lokalizacje)))
                    EditLocation.Visibility = ViewStates.Gone;

                if (Operation == Operation.In ? (Item.Base.idFunkcjiLogistycznejP >= 0) : (Item.Base.idFunkcjiLogistycznejW >= 0))
        ...
        private void EditPartia_Click(object sender, EventArgs e)
        {
            RunIsBusyAction(() =>
            {
                ...
                i.PutExtra(PartieActivity.Vars.IDMagazynu, Operation == Operation.In ? Dokument.intMagazynP : Dokument.intMagazynW);
        ...
        private void EditPaleta_Click(object sender, EventArgs e)
        {
            RunIsBusyAction(() =>
            {
                ...
                i.PutExtra(PaletyActivity.Vars.IDMagazynu, Operation == Operation.In ? Dokument.intMagazynP : Dokument.intMagazynW);
        ...
        private void EditSerial_Click(object sender, EventArgs e)
        {
            RunIsBusyAction(() =>
            {
                Intent i = new Intent(this, typeof(SerialActivity));
                    ...
                    i.PutExtra(PaletyActivity.Vars.IDTowaru, (int)Article.Tag);
                    i.PutExtra(PaletyActivity.Vars.IDLokalizacji, (int)Location.Tag);
                StartActivityForResult(i, (int)DocumentItemActivity_Common.ResultCodes.SerialActivityResult);
            });
        }
        ...
        private void GetAndSetControls()
        {
            ...
            switch (Operation)
            {
                case Operation.In: Scanhint.Text = GetString(Resource.String.documentitem_activity_scanhint_loc_add); break;
                case Operation.Out: Scanhint.Text = GetString(Resource.String.documentitem_activity_scanhint_loc_take); break;
            }

            FindViewById<FloatingActionButton>(Resource.Id.document_item_btn_cancel).Click += Cancel_Click;
            FloatingActionButton OKButton = FindViewById<FloatingActionButton>(Resource.Id.document_item_btn_ok);
            ...
            EditLocation = FindViewById<FloatingActionButton>(Resource.Id.document_item_btn_location);
            ...
            CanBeAddedToLoc = FindViewById<TextView>(Resource.Id.document_item_amountcanbeaddedtoloc);
            ...
            DocumentItemActivity_Common.SetVisibilityOfFields(this, DocType, Dokument.bZlecenie, Mode);
            SetupFields();
        ...
        private PozycjaVO GetDBObject()
        {
            ...
            {
                if (Globalne.debug_mode == 1) { Console.WriteLine("[ARCHUNG!] - fail safe in progress... ! "); }

                if (Location.Text != null || Location.Text != "")
                {
                    if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Lokalizacja.Text:" + Location.Text); }
                    if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Lokalizacja.Tag:" + Location.Tag); }
                }

                przedTag1 = (string)Location.Tag;
                przedText1 = (string)Location.Text;
                przedLocId1 = Globalne.lokalizacjaBL.PobierzLokalizacjęWgNazwy(Location.Text, Globalne.Magazyn.ID).ID;
                ...
                Task.Delay(50);
                Location.Tag = Globalne.lokalizacjaBL.PobierzLokalizacjęWgNazwy(Location.Text, Globalne.Magazyn.ID).ID;
                Task.Delay(50);
            ...
            if (Operation == Operation.In)
            {
                if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Operation" + Operation.ToString()); }
            ...
            try
            {
                Poz.dtDataProdukcji = DateTime.ParseExact(ProdDate.Text, Globalne.CurrentSettings.DateFormat, System.Globalization.CultureInfo.InvariantCulture);
                Poz.dtDataPrzydatności = DateTime.ParseExact(BestBefore.Text, Globalne.CurrentSettings.DateFormat, System.Globalization.CultureInfo.InvariantCulture);
            }
            catch (Exception)
            {
        ...
        private async Task<bool> CheckSanity()
        {
            ...
            {
                await Helpers.AlertAsyncWithConfirm(this, Resource.String.documentitem_cant_be_zero);
                return false;
            }
            ...
            foreach (DocumentItemFields Key in Dict.Keys)
            {
                switch (Key)
                {
                    case DocumentItemFields.Symbol: OK = (!Dict[Key] || (int)Symbol.Text.Length > 0); break;
                    case DocumentItemFields.Article: OK = (!Dict[Key] || (int)Article.Tag >= 0); break;
                    case DocumentItemFields.Flog: OK = (!Dict[Key] || !Globalne.CurrentSettings.FunkcjeLogistyczne || (int)Flog.Tag >= 0); break;
                    case DocumentItemFields.Location: OK = (!Dict[Key] || (int)Location.Tag >= 0); break;
                    ...
                    case DocumentItemFields.Owner: OK = (!Dict[Key] || (int)Owner.Tag >= 0); break;
                    ...
                    case DocumentItemFields.Unit: OK = (!Dict[Key] || (int)Unit.Tag >= 0); break;
                    case DocumentItemFields.KodEAN: OK = (!Dict[Key] || KodEAN.Text != ""); break;
                    case DocumentItemFields.NrKat: OK = (!Dict[Key] || NrKat.Text != ""); break;
                }
            ...
            await RunIsBusyTaskAsync(async () =>
             {
                 try
                 {
                         ...
                         case ItemActivityMode.Create:
                             {
                                 ...
                                 if (Globalne.CurrentSettings.OnlyOncePalleteSSCCOnDocument
                                    && (DocType == DocTypes.PZ || DocType == DocTypes.PW)
                                    && Globalne.paletaBL.CzyPaletaIstnieje(Poz.strPaletaP, -1))
                                 {
                                     await Helpers.Alert(this, "Ustawienia wdrożeniowe nie pozwalają na dodanie drugiej takiej samej palety.\nPaleta (SSCC) o takim kodzie jest już w systemie.", Title: "Uwaga!");
                                     return;
                                 }
                                 ...
                                 // todo fancy  mode 
                                 int IDPozIst = Globalne.dokumentBL.PobierzIDPozycjiIdentycznejNaDokumencie(Poz);
                                 ...
                                 if (IDPozIst != -1)
                                 {
                                     ...
                                     Res = Globalne.dokumentBL.EdytujPozycję(Helpers.StringDocType(DocType), PozIst, Dokument.bIgnorujBlokadePartii);
                                 }
                                 ...
                                 if (IDPozIst < 0 || Res < 0)
                                 {
                                     ...
                                     AutoException.ThrowIfNotNull(this, ErrorType.ItemCreationError, Globalne.dokumentBL.ZróbPozycję(Helpers.StringDocType(DocType), Poz, Dokument.bIgnorujBlokadePartii));
                         ...
                         case ItemActivityMode.Edit:
                              {
                                 ...
                                 if (Dokument.bZlecenie && Poz.numIloscZrealizowana > Poz.numIloscZlecona && !Dokument.bDopuszczalnePrzekroczenieIlosci)
                                     AutoException.ThrowIfNotNull(this, Resource.String.documentitem_creationedition_toomuch);

                                 AutoException.ThrowIfNotNull(this, ErrorType.ItemCreationError, Globalne.dokumentBL.EdytujPozycję(Helpers.StringDocType(DocType), Poz, Dokument.bIgnorujBlokadePartii));
                                 break;
                             }
                         ...
                         case ItemActivityMode.Split:
                             {
                                 ...
                                 else
                                 {
                                     ...
                                     BusinessLogicHelpers.DocumentItems.EditSplitItem(this, Original, true, DocType, Operation, Poz.numIloscZrealizowana);
                                     ...
                                     catch (Exception ex)
                                     {
                                         ...
                                         BusinessLogicHelpers.DocumentItems.EditSplitItem(this, Original, false, DocType, Operation);
                                         return;
                                     }
                                 }
                     ...
                     if (LastScanData != null)
                         i.PutExtra(DocumentItemActivity_Common.Results.WereScanned, LastScanData.ToArray());
        ...
        private async void OK_Click_ZLMM(object sender, EventArgs e)
        {
            ...
            
            {
                ...
                if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Getting DBObject"); }
                
                PozycjaVO Poz = GetDBObject();

                if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: DBObject Received"); }
                ...
                switch (Mode)
                {
                    case ItemActivityMode.Create:
                        {
                            if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Checking the same position on document"); }
                            ...
                            if (IDPozIst == -1)
                            {
                                if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Getting the same position once again"); }

                                IDPozIst = Globalne.dokumentBL.PobierzIDPozycjiIdentycznejNaDokumencie(Poz);
                            }

                            if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Received document position: " + IDPozIst.ToString()); }
                            ...
                            if (IDPozIst != -1)
                            {
                                ...
                                if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Editing the same position"); }
                            ...
                            if (IDPozIst < 0 || Res < 0)
                            {
                                ...
                                if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: The same position not found - Creating new position"); }
                    ...
                    case ItemActivityMode.Edit:
                        {
                            if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: Editing position"); }
                            
                            if ((DocType == DocTypes.ZLDistribution || DocType == DocTypes.ZL) && IsBlockedPosition)
                            {
                                await Helpers.Alert(this, "Pozycja tylko w trybie do odczytu. Zamiany beda odrzucone.");
                                break;
                            }
                            ...
                            if (Operation == Operation.Out)
                            {
                                ...
                                if (Dokument.bZlecenie && Poz.numIloscZebrana > Poz.numIloscZlecona && !Dokument.bDopuszczalnePrzekroczenieIlosci)
                                    AutoException.ThrowIfNotNull(this, Resource.String.documentitem_creationedition_toomuch);
                            }
                            ...
                            if (Poz.numIloscZrealizowana > Poz.numIloscZebrana)
                                AutoException.ThrowIfNotNull(this, Resource.String.documentitem_edition_zlmm_distrib_more_than_gather);
                    ...
                    case ItemActivityMode.Split:
                        {
                            ...
                            if (Operation == Operation.Out)
                            {
                                ...
                                {
                                    Poz.numIloscZlecona = NumAmount.Value;
                                    BusinessLogicHelpers.DocumentItems.EditSplitItem(this, Original, true, DocType, Operation, Poz.numIloscZebrana);
                            ...
                            else if (Operation == Operation.In)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        Helpers.HandleError(this, ex);
                                        BusinessLogicHelpers.DocumentItems.EditSplitItem(this, Original, true, DocType, Operation);
                                    }
                                }
                            }
        ...
        private void EditOwner_Click(object sender, EventArgs e)
        {
            RunIsBusyAction(() =>
            {
                ...
                StartActivityForResult(i, (int)DocumentItemActivity_Common.ResultCodes.ContractorsActivityResult);
            });
        }
        ...
        private void EditLocation_Click(object sender, EventArgs e)
        {
            RunIsBusyAction(() =>
            {
                ...
                i.PutExtra(LocationsActivity.Vars.IDMagazynu, Operation == Operation.In ? Dokument.intMagazynP : Dokument.intMagazynW);
                ...
                StartActivityForResult(i, (int)DocumentItemActivity_Common.ResultCodes.LocationsActivityResult);
            });
        ...
        {
            await RunIsBusyTaskAsync(() => BusinessLogicHelpers.Indexes.ShowLogisticFunctionsListAndSet(this, Operation == Operation.In ? Dokument.intMagazynP : Dokument.intMagazynW, Flog));
        ...
        async void EditUnit_Click(object sender, EventArgs e)
        {
            ...
            await RunIsBusyTaskAsync(() => BusinessLogicHelpers.Indexes.ShowUnitListAndSet(this, (int)Article.Tag, Unit));
        ...
        private void EditArticle_Click(object sender, EventArgs e)
        {
            RunIsBusyAction(() =>
            {
                ...
                i.PutExtra(ArticlesActivity.Vars.IDMagazynu, Operation == Operation.In ? Dokument.intMagazynP : Dokument.intMagazynW);
                ...
                StartActivityForResult(i, (int)DocumentItemActivity_Common.ResultCodes.ArticlesActivityResult);
            });
        }
        ...
        {
            if (Paleta.Text != "" && Globalne.CurrentSettings.GetDataFromFirstSSCCEntry && Globalne.CurrentSettings.Palety)
                if (BusinessLogicHelpers.DocumentItems.GetSSCCData(Paleta.Text, ref Item, Operation))
                {
                    if (Item.ExPartia != "")
                    {
                        Partia.Text = Item.ExPartia;
        ...
        private void Do_Refresh(bool InWarehouseOnly = false)
        {
            ...
            if (!InWarehouseOnly)
            {
                ...
                if (Set[DocumentItemDisplayElements.SerialNumber] && Globalne.CurrentSettings.SerialNumber)
                {
            ...
            if (Set[DocumentItemDisplayElements.CanBeAddedToLoc])
            {
                ...
                {
                    int Value = Globalne.przychrozchBL.ObliczIleCałkTowaruZmieściSięWLokalizacji((int)Article.Tag, (int)Unit.Tag, (int)Location.Tag);

                    Helpers.SetTextOnTextView(this, CanBeAddedToLoc, Value == -1 ? "∞" : Value.ToString("F3"));
                }
                else
                ...
                if (resultCode == Result.Ok)
                {
                    switch (requestCode)
                    {
                        case (int)DocumentItemActivity_Common.ResultCodes.ArticlesActivityResult:
                            {
                                TowarVO Art = (TowarVO)Helpers.DeserializePassedJSON(data, ArticlesActivity.Results.SelectedJSON, typeof(TowarVO));
                                ...
                                if (Art != null && Art.ID >= 0)
                                {
                                    ...
                                    JednostkaMiaryO Jedn = Globalne.towarBL.PobierzJednostkęDomyślnąTowaru(Art.ID);
                                    Unit.Tag = Jedn.ID;
                        ...
                        case (int)DocumentItemActivity_Common.ResultCodes.LocationsActivityResult:
                            {
                                LokalizacjaVO Lok = (LokalizacjaVO)Helpers.DeserializePassedJSON(data, LocationsActivity.Results.SelectedJSON, typeof(LokalizacjaVO));
                        ...
                        case (int)DocumentItemActivity_Common.ResultCodes.ContractorsActivityResult:
                            {
                                KontrahentVO Contr = (KontrahentVO)Helpers.DeserializePassedJSON(data, ContractorsActivity.Results.SelectedJSON, typeof(KontrahentVO));
                        ...
                        case (int)DocumentItemActivity_Common.ResultCodes.SerialActivityResult:
                            {
                                NumerSeryjnyO Ser = (NumerSeryjnyO)Helpers.DeserializePassedJSON(data, SerialActivity.Results.SelectedJSON, typeof(NumerSeryjnyO));
        ...
        public override bool DispatchKeyEvent(KeyEvent e)
        {
            ...
            {
                if (BusinessLogicHelpers.DocumentItems.IsDistributionMode(DocType, Operation) || BusinessLogicHelpers.DocumentItems.IsGatheringMode(DocType, Operation))
                {
        ...
        protected override async Task<bool> CheckBeforeAssumingScanningPath(List<string> BarcodesL)
        {
            ...
            int Lokalizacja = Operation == Operation.In ? Item.Base.idLokalizacjaP : Item.Base.idLokalizacjaW;
            ...
            if (Loc.ID > -1 && !Globalne.CurrentSettings.AllowSourceLocationChange && ((!Globalne.CurrentSettings.Lokalizacje) || (Lokalizacja >= 0) && (Lokalizacja != Loc.ID)))
            {
                await Helpers.AlertAsyncWithConfirm(this, Resource.String.documentitem_location_already_set, Resource.Raw.sound_error);
                LastScanData = null;
                return false;
            }
        ...
        private void DoBarcode()
        {
            ...
            try
            {
                ...
                {
                    var czyLokalizacja = Globalne.lokalizacjaBL.PobierzLokalizacjęWgKoduKreskowego(LastScanData[0], Globalne.Magazyn.ID, true)?.ID != -1;
                    var skanowanyTowarNieIstnieje = Globalne.kodykreskoweBL.WyszukajKodKreskowy(LastScanData[0])?.Towar == "";
                    ...
                    {
                        var skanowanyTowar = Globalne.towarBL.PobierzTowarWgKoduKreskowego(LastScanData[0]).FirstOrDefault();
                        // sprawdzamy czy towar jest na liscie pozycji w danym dokumencie
                        if (Dokument.bZlecenie && Globalne.dokumentBL.PobierzListęIDPozycji(Dokument.ID).Select(x => x == skanowanyTowar.ID).Count() == 0)
                        {
                            AutoException.ThrowIfNotNull(this, Resource.String.articles_cannot_be_used);
                        }
                        // jezeli zaznaczona jest opcja w ustawieniach - pomin potwierdzanie pozycji kolejnym towarem
                        if ((DocType.Equals(DocTypes.RW) && Globalne.CurrentSettings.SkipConfirmPositionRW)
                            || (DocType.Equals(DocTypes.PW) && Globalne.CurrentSettings.SkipConfirmPositionPW)
                            || (DocType.Equals(DocTypes.PZ) && Globalne.CurrentSettings.SkipConfirmPositionPZ)
                            || (DocType.Equals(DocTypes.WZ) && Globalne.CurrentSettings.SkipConfirmPositionWZ))
                        {
                            LastScanData = null;
                            return;
                        }
                ...
                if (BusinessLogicHelpers.DocumentItems.IsDistributionMode(DocType, Operation) || BusinessLogicHelpers.DocumentItems.IsGatheringMode(DocType, Operation))
                {
                    if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: OK_Click_ZLMM entered"); }
                    if (Operation != Operation.In)
                ...
                {
                    if (Globalne.debug_mode == 1) { Console.WriteLine("DEBUG_INFO: OK_Click entered"); }
                    OK_Click(null, null);
                }
            }
            catch (Exception ex)
