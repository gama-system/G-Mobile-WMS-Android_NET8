...
namespace G_Mobile_Android_WMS.BusinessLogicHelpers
{
    public static class Documents
    {
        ...
        public static int CreateDocument(Context ctx, Enums.DocTypes DocType, int RegistryID, int ContractorID,
                                         int SrcFlogID, int TarFlogID, int TarWarehouse, int Buffer, int LokDoc,
                                         string RelatedDoc, string Description)
        {
            if (RegistryID < 0)
                throw new Exception(ctx.GetString(Resource.String.creating_documents_registry_must_be_set));

            StatusDokumentuO Status = Globalne.dokumentBL.PobierzPierwszyStatusDokumentuOTypie(Helpers.StringDocType(DocType), (int)Enums.DocumentStatusTypes.WRealizacji);
            ...
            Dokument.intPriorytet = Globalne.rejestrBL.PobierzRejestr(Dokument.idRejestr).intPriorytet;
            ...
            switch (DocType)
            {
                ...
                default:
                    throw new Exception(ctx.GetString(Resource.String.creating_documents_cannot_create_doc_of_type));
            }
        ...
        public static async Task<bool> ShowAndApplyInventoryLocationExitOptions(Context ctx, DokumentVO Doc, int IDInvLoc, bool SetClosedWithoutAsking = false)
        {

            if (!SetClosedWithoutAsking)
            {
                string[] Options = {
                                        ctx.GetString(Resource.String.editingdocuments_inventoryloc_leave_without_marking),
                                        ctx.GetString(Resource.String.editingdocuments_inventoryloc_leave_markcomplete),
                                   };

                string Res = await UserDialogs.Instance.ActionSheetAsync(ctx.GetString(Resource.String.editingdocuments_inventoryloc_leave), ctx.GetString(Resource.String.global_cancel), "", null, Options);

                if (Res == ctx.GetString(Resource.String.global_cancel))
                    return false;
                else if (Res == ctx.GetString(Resource.String.editingdocuments_inventoryloc_leave_without_marking))
                {
                    return true;
                }
                else
        ...
        public static async Task<bool> ShowAndApplyDocumentExitOptions(Context ctx, List<DokumentVO> Documents, Enums.DocTypes DocType, Enums.DocumentLeaveAction Action = DocumentLeaveAction.None, bool AskConfirm = true, bool Multipicking = false)
        {
            ...
            if (Action == DocumentLeaveAction.None)
                Res = await UserDialogs.Instance.ActionSheetAsync(ctx.GetString(Resource.String.editingdocuments_leave), ctx.GetString(Resource.String.global_cancel), "", null, przyciski);
            ...
            if (Res == ctx.GetString(Resource.String.global_cancel))
                return false;
            else if (Res == ctx.GetString(Resource.String.editingdocuments_leave_allow_others_to_edit) || Action == DocumentLeaveAction.Leave)
            {

                Action = DocumentLeaveAction.Leave;

                if (Globalne.CurrentSettings.StatusesToSetOnDocumentLeave[DocType] >= 0)
                    StatusOK = Globalne.dokumentBL.PobierzStatusDokumentu(Globalne.CurrentSettings.StatusesToSetOnDocumentLeave[DocType]);

                if (StatusOK == null || StatusOK.ID < 0)
                    StatusOK = Globalne.dokumentBL.PobierzPierwszyStatusDokumentuOTypie(Helpers.StringDocType(DocType), (int)Enums.DocumentStatusTypes.DoRealizacji);

                if (StatusOK.ID < 0)
                    AutoException.ThrowIfNotNull(ctx, Resource.String.global_define_statuses);
            }
            else if (Res == ctx.GetString(Resource.String.editingdocuments_leave_allow_only_me_to_edit) || Action == DocumentLeaveAction.Pause)
            {
                Action = DocumentLeaveAction.Pause;

                if (Globalne.CurrentSettings.StatusesToSetOnDocumentLeave[DocType] >= 0)
                    StatusOK = Globalne.dokumentBL.PobierzStatusDokumentu(Globalne.CurrentSettings.StatusesToSetOnDocumentPause[DocType]);
                else
                    StatusOK = null;
            }
            else if (Res == ctx.GetString(Resource.String.editingdocuments_leave_closedocument) || Action == DocumentLeaveAction.Close)
            {
                if (AskConfirm)
                {
                    bool Resp = await Helpers.QuestionAlertAsync(ctx,
                    ...
                    else
                    {
                        ...
                        var drukarkiEtykiet = Globalne.drukarkaBL.PobierzDrukarkeEtykiet().strERP.TrimEnd();
                        var drukarkiEtykietList = string.IsNullOrEmpty(drukarkiEtykiet) ? new List<string>() : drukarkiEtykiet.Split(';').ToList();

                        var numeryDrukarkiEtykiet = new List<int>();

                        // dostep do etykiet przez tylko wybranych operatorow
                        var operatorzyEtykiet = Globalne.drukarkaBL.PobierzOperatorowEtykiet().strNazwa ?? "";
                        // jezeli brak wpisow z operatorami uznajemy ze wszyscy moga drukowac etykiety
                        bool czyOperatorPrzypisany = ((!string.IsNullOrEmpty(operatorzyEtykiet) && operatorzyEtykiet.Contains(Globalne.Operator.Login)) || string.IsNullOrEmpty(operatorzyEtykiet));

                        // rejestry do wydruku etykiet
                        /// skladnia: rejestr, numer drukarki - RW/WEW,2;PZ/OSO,1
                        var rejestryEtykiet = Globalne.drukarkaBL.PobierzRejestryEtykiet().strNazwa ?? "";
                        var rejestryEtykietList = string.IsNullOrEmpty(rejestryEtykiet) ? new List<string>() : rejestryEtykiet.Split(';').ToList();
                        ...
                        {
                            // [0] nazwa rejestru
                            var rejestDokumentu = Globalne.rejestrBL.PobierzRejestr(Documents.FirstOrDefault().idRejestr);
                            if (rejestr.Split(',')[0] == rejestDokumentu.strTyp + "/" + rejestDokumentu.strSymbolRej)
                            {
                                czyRejestrZgodnyZDokumentem = true;
                                int numerDrukarki = 0;
                                // [1] numer drukarki
                                if (rejestr.Split(',')[1] != "*" && int.TryParse(rejestr.Split(',')[1], out numerDrukarki))
                        ...
                        if (Etykieta.strNazwa.Contains("1")
                            && czyOperatorPrzypisany
                            && czyRejestrZgodnyZDokumentem)
                        {
                            bool Resp2 = await Helpers.QuestionAlertAsyncEtykieta(ctx,
                            ...
                            if (Resp2)
                            {
                                ...
                                if (PrintIsPossible)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            Globalne.dokumentBL.WydrukEty(Documents.FirstOrDefault().strNazwa, location, numerDrukarki);
                                        }
                                        //return true;
                                    }
                                    catch (Exception)
                ...
                if (!Multipicking)
                {
                    if (Globalne.CurrentSettings.StatusesToSetOnDocumentFinish[DocType] >= 0)
                        StatusOK = Globalne.dokumentBL.PobierzStatusDokumentu(Globalne.CurrentSettings.StatusesToSetOnDocumentFinish[DocType]);

                    if (StatusOK == null || StatusOK.ID < 0)
                        StatusOK = Globalne.dokumentBL.PobierzPierwszyStatusDokumentuOTypie(Helpers.StringDocType(DocType), (int)Enums.DocumentStatusTypes.Zamknięty);
                }
                ...
                {
                    if (Globalne.CurrentSettings.StatusesToSetOnDocumentFinish[DocType] >= 0)
                        StatusOK = Globalne.dokumentBL.PobierzStatusDokumentu(Globalne.CurrentSettings.StatusesToSetOnDocumentDone[DocType]);

                    if (StatusOK == null || StatusOK.ID < 0)
                        StatusOK = Globalne.dokumentBL.PobierzPierwszyStatusDokumentuOTypie(Helpers.StringDocType(DocType), (int)Enums.DocumentStatusTypes.Wykonany);
                }
                ...
                if (Globalne.CurrentSettings.StatusesToSetOnDocumentFinishIncorrect[DocType] >= 0)
                    StatusNOK = Globalne.dokumentBL.PobierzStatusDokumentu(Globalne.CurrentSettings.StatusesToSetOnDocumentFinishIncorrect[DocType]);

                if (StatusNOK == null || StatusNOK.ID < 0)
                    StatusNOK = Globalne.dokumentBL.PobierzPierwszyStatusDokumentuOTypie(Helpers.StringDocType(DocType), (int)Enums.DocumentStatusTypes.Wstrzymany);
                    ...
                    case DocumentLeaveAction.Leave:
                        {
                            ...
                            if (!Doc.bZlecenie && (Doc.intUtworzonyPrzez == Globalne.Operator.ID || Globalne.CurrentUserSettings.CanDeleteAllDocuments))
                            {
                                // blokowanie usuwania dokumentow IN bez pozycji
                                if (Globalne.dokumentBL.PobierzListęIDPozycji(Doc.ID).Count == 0 && DocType != DocTypes.IN)
                                {
                                    Globalne.dokumentBL.UsuńDokument(Doc.ID);
                                    Helpers.CenteredToast(ctx, Resource.String.document_was_empty_and_was_deleted, ToastLength.Short);
                                }
                            }
            ...
            if (Action == DocumentLeaveAction.Close)
            {
                if (AskConfirm)
                {
                    ...
                    {
                        bool S = await Helpers.QuestionAlertAsync(ctx,
                                                                  ctx.GetString(Resource.String.editingdocuments_leave_incomplete) + " '" + StatusNOK?.strNazwaStatusu + "'?",
                                                                  Resource.Raw.sound_message);
                ...
                foreach (DokumentVO Doc in Documents)
                {
                    ...
                    if (Multipicking && Globalne.CurrentSettings.MultipickingAutoKompletacjaAfterFinish)
        ...
        public static object[] GetDocuments(Context ctx, List<int> DocIDs)
        {
            ...
            foreach (int ID in DocIDs)
            {
                ...
                if (Doc.ID < 0)
                    ...
                    AutoException.ThrowIfNotNull(ctx, Resource.String.documents_cantedit_many_of_this_type);
                ...
                if (Status == Enums.DocumentStatusTypes.Otwarty)
                    AutoException.ThrowIfNotNull(ctx, Resource.String.documents_cantedit_opened_or_closed);

                if (DocIDs.Count() != 1 && Status == Enums.DocumentStatusTypes.Zamknięty)
                    AutoException.ThrowIfNotNull(ctx, Resource.String.documents_cantedit_many_if_closed);
...
#warning HiveInvoke
                string TypDoc = (string)Helpers.HiveInvoke(typeof(WMSServerAccess.Dokument.DokumentBL), "PobierzTypDokumentu", new object[] { Doc.ID, "", "", -1, -1, "" });
                DocTypes DocType = (DocTypes)Enum.Parse(typeof(DocTypes), TypDoc);

                if (DocType == DocTypes.PW || DocType == DocTypes.PZ || DocType == DocTypes.MM || DocType == DocTypes.ZL || DocType == DocTypes.IN)
                {
                    if (Doc.intMagazynP != Globalne.Magazyn.ID)
                        AutoException.ThrowIfNotNull(ctx, Resource.String.documents_wrong_warehouse);
                }
        ...
        public static bool SetDocumentsStatusOnEnter(Context ctx, List<DokumentVO> Documents, Enums.DocTypes DocType)
        {
            StatusDokumentuO Status;

            if (Globalne.CurrentSettings.StatusesToSetOnDocumentEnter[DocType] < 0)
                Status = Globalne.dokumentBL.PobierzPierwszyStatusDokumentuOTypie(Helpers.StringDocType(DocType), (int)Enums.DocumentStatusTypes.WRealizacji);
            else
                Status = Globalne.dokumentBL.PobierzStatusDokumentu(Globalne.CurrentSettings.StatusesToSetOnDocumentEnter[DocType]);
            ...
            else
            {
                foreach (DokumentVO Doc in Documents)
                {
                    Globalne.dokumentBL.UstawOperatoraEdytującegoDokument(Doc.ID, Globalne.Operator.ID);

                    if (Globalne.dokumentBL.PobierzTypStatusuDokumentu(Doc.ID, "", "", -1, -1, "") <= (int)DocumentStatusTypes.Wykonany)
        ...
        public static async Task<bool> DeleteDocuments(Context ctx, List<int[]> Docs)
        {
            ...
            else if (Docs.Count == 1)
            {
                ...
                if (!Globalne.CurrentUserSettings.CanDeleteAllDocuments && (IDOfCreator != Globalne.Operator.ID))
                {
                    AutoException.ThrowIfNotNull(ctx, Resource.String.documents_cantdelete_notyours);
                    return false;
                }
                if (!Globalne.CurrentUserSettings.CanDeleteClosedDocuments)
                {
                    ...
                    {
                        int Status = Globalne.dokumentBL.PobierzTypStatusuDokumentu(Doc[1], "", "", -1, -1, "");

                        if (Status == (int)DocumentStatusTypes.Zamknięty || Status == (int)Enums.DocumentStatusTypes.Wstrzymany)
                        {
                            AutoException.ThrowIfNotNull(ctx, Resource.String.documents_cantdelete_closed);
                            return false;
                        }
                    }
                }

                bool Res = await Helpers.QuestionAlertAsync(ctx, Resource.String.documents_delete, Resource.Raw.sound_message);

                if (Res)
                {
                    AutoException.ThrowIfNotNull(ctx, ErrorType.DocumentDeletionError, Globalne.dokumentBL.UsuńDokument(IDDoc));
                    return true;
                }
                else
                    return false;
            ...
            {
                if (!await Helpers.QuestionAlertAsync(ctx, Resource.String.documents_deletemany, Resource.Raw.sound_message) ||
                    !await Helpers.QuestionAlertAsync(ctx, Resource.String.documents_deletemanyconfirm, Resource.Raw.sound_alert))
                    return false;
                else
                {
                    ...
                    if (ErrorsHappened)
                        await Helpers.AlertAsyncWithConfirm(ctx, Resource.String.documents_cantdelete_many);
        ...
        public static async void EditDocuments(BaseWMSActivity ctx, List<int> DocIds, DocTypes DocType, ZLMMMode ZLMMMode,
                                         int InventoryLoc = -1, List<DokumentVO> PreDownloadedDocList = null, bool Multipicking = false)
        {
            ...
            if (Multipicking && (DocType == DocTypes.WZ || DocType == DocTypes.RW))
            {
                ...
                i.PutExtra(EditingDocumentsActivity_Common.Vars.DocsJSON, Helpers.SerializeJSON((List<DokumentVO>)Docs[1]));
            }
            else if (DocType == DocTypes.IN && InventoryLoc < 0)
            {
                i = new Intent(ctx, typeof(InventoryLocationsActivity));
                i.PutExtra(InventoryLocationsActivity.Vars.InventoryDoc, Helpers.SerializeJSON((Docs[1] as List<DokumentVO>)[0]));
            }
            else
            {
                if (DocType == DocTypes.MM || DocType == DocTypes.ZL || DocType == DocTypes.ZLDistribution || DocType == DocTypes.ZLGathering)
                {
                    if (Globalne.CurrentSettings.DefaultZLMode == ZLMMMode.None && ZLMMMode == ZLMMMode.None)
                    {
        ...
        public static async Task<Enums.ZLMMMode> AskZLMMMode(Activity ctx, DocTypes DocType)
        {
            ...
            else if (DocType == DocTypes.MM && Globalne.CurrentSettings.DefaultMMMode != ZLMMMode.None)
            ...
            string Res = await UserDialogs.Instance.ActionSheetAsync(ctx.GetString(Resource.String.editingdocuments_zlmm_howtoedit),
        ...
        public static async Task<Enums.WZRWMode> AskWZRWMode(Activity ctx, DocTypes DocType)
        {
            ...
            string Res = await UserDialogs.Instance.ActionSheetAsync(ctx.GetString(Resource.String.editingdocuments_rwzm_howtoedit),
                                                                     ctx.GetString(Resource.String.global_cancel),
                                                                     "",
                                                                     null,
                                                                     Options);
        ...
        public static async Task<Enums.WZRWContMode> AskWZRWContinuation(Activity ctx, DocTypes DocType)
        {
                   string[] Options = {
        ...
        public static async Task<Boolean> AskYesOrNo(Activity ctx, String question)
        {
            ...
            string Res = await UserDialogs.Instance.ActionSheetAsync(question,
                                                                     ctx.GetString(Resource.String.global_cancel),
                                                                     "",
                                                                     null,
                                                                     Options);
